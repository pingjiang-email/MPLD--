...
import torch
from trainer.utils import LambdaLR,Logger,ReplayBuffer,ToTensor,Resize3D
import torch.nn.functional as F
from trainer.utils import Logger
import numpy as np
from trainer.Eva_datasets import ImageDataset
from models.Eva_model import Evaluator
...
class Eva_Trainer():
    def __init__(self, config):
        ...
        self.dataloader = DataLoader(ImageDataset(config['dataroot'],transforms_=self.transforms_1,
                           opt = config, unaligned=False),batch_size=config['batchSize'],shuffle=False,num_workers=config['n_cpu'])
    ...
    def train(self):
        ...
        for epoch in range(self.config['epoch'], self.config['n_epochs']):
            ...
            for A1_noise,A2,Label in self.dataloader:
                A1_noise = cv2.morphologyEx(A1_noise, cv2.MORPH_OPEN, kernel)
